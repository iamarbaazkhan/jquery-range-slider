...
function slider(s_id) {
  ...
  var getTrackStyle = function (elem) {
    ...
    console.log(
      "getTrackStyle - " + labelsRange + " = " + curLabel.data("stop")
    );
    ...
    var style =
      range +
      " {background: linear-gradient(to right, var(--slider-active-color) 0%, var(--slider-active-color) " +
      seltrack +
      "%, " +
      $(range).parent().css("background-color") +
      " " +
      seltrack +
      "%, " +
      $(range).parent().css("background-color") +
